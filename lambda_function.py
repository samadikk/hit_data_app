...
class PrepData:
    ...
    def get_medium(self, mlist):
        ...
        Function get_medium() accepts 'referrer' column from the original dataset
            ...
            If the referrer does not contain either of Google, Facebook or Yahoo
    ...
    def process_data(self, data, bucket):
        """
            ...
            Follwing are the sequence of steps:
                ...
                c) Revenue field is then extracted from 'product_list' from original dataset.
                d) Finally results are aggregated by new columns 'Medium' and 'Keyword'
                e) The header row of the aggregated dataset (data_result) is renamed
        ...
        if data is not None:
            ...
            data['medium'] = data.groupby(['ip', 'user_agent', 'geo_city', 'geo_region', 'geo_country'])[
                'medium'].fillna(method='ffill')
            data['medium'] = data.groupby(['ip', 'user_agent', 'geo_city', 'geo_region', 'geo_country'])[
                'medium'].fillna(method='bfill')
            ...
            data_result.rename(
                columns={'medium': 'Search Engine Domain', 'keyword': 'Search Keyword', 'revenue': 'Revenue'},
                inplace=True)
